...
// These sentinels exceed Number.MAX_SAFE_INTEGER on purpose: they're compared
// against pointer values that bun:ffi also returns as imprecise JS numbers at
// this magnitude, so the lossy comparison is the intended behavior.
// biome-ignore lint/correctness/noPrecisionLoss: pointer sentinel near 2^64
...
const cachedHelperFp = (
  await Bun.file(helperFpPath)
    .text()
    .catch(() => "")
).trim();
...
const mapShm = (
  ...
  size: number,
  create: boolean,
): { pointer: number; buffer: Uint8Array } => {
  ...
  const buffer = toBuffer(pointer as Pointer, 0, size);
...
export const signalToBun = (region: SharedRegion): void => {
  const r = sys.sem_post(region.semToBun as Pointer) as number;
  if (r !== 0) {
    throw new Error("sem_post failed on semToBun");
  }
};

export const signalToShim = (region: SharedRegion): void => {
  const r = sys.sem_post(region.semToShim as Pointer) as number;
  if (r !== 0) {
    throw new Error("sem_post failed on semToShim");
  }
};

export const waitForBunSignal = (region: SharedRegion): void => {
  const r = sys.sem_wait(region.semToBun as Pointer) as number;
  if (r !== 0) {
    throw new Error("sem_wait failed on semToBun");
  }
};

export const tryWaitForShimSignal = (region: SharedRegion): boolean => {
  const r = sys.sem_trywait(region.semToShim as Pointer) as number;
  return r === 0;
};
