...
export const signalToBun = (region: SharedRegion): void => {
  const r = pt.sem_post(region.semToBun as Pointer) as number;
  if (r !== 0) {
...
export const signalToShim = (region: SharedRegion): void => {
  const r = pt.sem_post(region.semToShim as Pointer) as number;
  if (r !== 0) {
...
export const waitForBunSignal = (region: SharedRegion): void => {
  const r = pt.sem_wait(region.semToBun as Pointer) as number;
  if (r !== 0) {
...
export const tryWaitForShimSignal = (region: SharedRegion): boolean => {
  const r = pt.sem_trywait(region.semToShim as Pointer) as number;
  return r === 0;
};
