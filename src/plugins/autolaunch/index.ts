...
const appId = (): string => {
  const id = (process.env.BUTTER_APP_ID || process.env.BUTTER_TITLE || "butterapp").replace(
    /[^a-zA-Z0-9._-]/g,
    "_",
  );
  if (!SAFE_ID.test(id)) {
...
const isEnabled = async (): Promise<{ ok: boolean; enabled: boolean }> => {
  try {
    if (process.platform === "darwin") {
      return { ok: true, enabled: existsSync(macPlistPath()) };
    }
    if (process.platform === "linux") {
      return { ok: true, enabled: existsSync(linuxDesktopPath()) };
    }
    if (process.platform === "win32") {
      const id = appId();
      try {
        const out =
          await Bun.$`reg query HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run /v ${id}`.text();
