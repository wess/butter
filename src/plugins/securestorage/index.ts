...
const escapePowerShell = (s: string): string => s.replace(/'/g, "''");
...
const storeSecret = async (service: string, key: string, value: string): Promise<void> => {
  ...
  if (platform === "darwin") {
    ...
    await Bun.$`secret-tool store --label=${key} service ${service} key ${key} < ${new Response(value)}`;
...
const retrieveSecret = async (service: string, key: string): Promise<string> => {
  ...
  const platform = process.platform;
  if (platform === "darwin") {
    const result = await Bun.$`security find-generic-password -s ${service} -a ${key} -w`.text();
    return result.trim();
  } else if (platform === "linux") {
