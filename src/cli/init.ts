...
// `butter init` fetches scaffolding templates over the network from the repo's
// `main` branch rather than from the installed package. This is deliberate: the
// npm tarball ships only `src` (see package.json `files`), so `templates/` is
// intentionally NOT bundled — keeping the install small and letting templates be
// updated without a release. The trade-off is that `init` requires connectivity
// and always pulls the latest `main` templates, not the ones matching the
// installed version.
