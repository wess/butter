...
export const distAppName = (config: Config): string =>
...
export const packageMacDmg = async (config: Config, projectDir: string): Promise<string> => {
...
const ensureAppImageTool = async (projectDir: string): Promise<string> => {
  ...
  if (!res.ok) {
    throw new Error(`appimagetool download failed: ${res.status}`);
  }
...
export const packageLinuxAppImage = async (config: Config, projectDir: string): Promise<string> => {
  const appName = distAppName(config);
...
export const nsisTemplate = (appName: string, exeName: string, identifier: string): string => `
...
export const packageWindowsZip = async (config: Config, projectDir: string): Promise<string> => {
  const appName = distAppName(config);
...
const packageWindowsNsis = async (config: Config, projectDir: string): Promise<string> => {
  const appName = distAppName(config);
  const _bundlePath = join(projectDir, "dist", appName);
